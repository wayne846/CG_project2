...
class Maze {
	...
	public:
		...
		void	Draw_View(const float, float modelMatrix[16], float perspectiveMatrix[16]);
		void Draw_Cell(Cell* cell, float frustumPoint1[2], float frustumPoint2[2], float modelMatrix[16], float perspectiveMatrix[16]);

		void Draw_Wall(const float start[2], const float end[2], const float color[3], float[16], float[16]);
